...
function getObjects_(data, keys) {
  ...
  for (var i = 0; i < data.length; ++i) {
    ...
    for (var j = 0; j < data[i].length; ++j) {
      ...
      //check if the key contains the string "image" or "video"
      if(keys[j].length >=5 && (keys[j].substring(0,5) == "image" || keys[j].substring(0,5) == "video" )){
        // if the key already exists we add the cellData to the already existing array of the value , if not existing we create a new array for this key
        if(keys[j].substring(0,5)+"s" in object){
          object[keys[j].substring(0,5)+"s"].push({"url":cellData});
        }else{
          object[keys[j].substring(0,5)+"s"] = [{"url":cellData}];
        }
      }else{
